...
## Rules
🎭 Player Archetypes The following strategies define how players interact within the network over multiple rounds:StrategyBehavior LogicCore BeliefFixedMaintains the same move every round regardless of neighbors' actions."My friends will eventually copy me; I do not need to change."AdaptiveCopies the move played by the majority of direct friends in the previous round."My friends only care about their own moves; I must adapt to them to coordinate."Forward LookingCopies the move played by the majority of their friends' friends in the previous round."My friends are copying their friends; I must look one step ahead to coordinate."Note: For both Adaptive and Forward Looking strategies, if there is no clear majority (a tie), the player defaults to playing Action G (or Action A) to attempt to initiate coordination.
